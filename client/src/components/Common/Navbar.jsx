...
const SearchBar = () => <input type="text" placeholder="Search..." className="p-1 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#ea2e0e] w-32 hidden lg:block transition duration-150" />;
const CartDrawer = ({ drawerOpen, toggleCartDrawer }) => (
    <div className={`fixed top-0 right-0 w-80 h-full bg-white shadow-2xl z-50 transform transition-transform duration-300 ${drawerOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="p-4 flex justify-between items-center border-b">
            <h2 className="text-lg font-semibold">Shopping Bag (4)</h2>
            <button onClick={toggleCartDrawer}><IoMdClose className="w-6 h-6" /></button>
        </div>
        <div className="p-4 text-sm text-gray-500">Cart items would be listed here.</div>
    </div>
);
...
const Navbar = () => {
  ...
  const userInitial = user?.name ? user.name.split(' ').map(n => n[0]).join('').toUpperCase().substring(0, 2) : 'U';
          ...
          <div className="flex items-center space-x-2">
            <Link to="/" className="flex items-center">
                <img 
                    ...
                    onError={(e) => { e.target.onerror = null; e.target.src = "https://placehold.co/120x28/ea2e0e/ffffff?text=ONE+MAN"; }}
                />
            ...
            <div className="relative">
                ...
                {isLoggedIn && profileMenuOpen && (
                    <div className="absolute right-0 mt-3 w-48 bg-white border border-gray-200 rounded-lg shadow-xl py-2 z-50 transform translate-x-1/4 md:translate-x-0">
                        ...
                        >
                             <HiOutlineArrowRightOnRectangle className="w-4 h-4 mr-2"/> Logout
                        </button>
                    </div>
                )}
            </div>
      ...
      >
        <div className="p-6">
          <div className="flex justify-between items-center mb-8 border-b pb-4">
            <div className="flex items-center">
                <img 
                    ...
                    onError={(e) => { e.target.onerror = null; e.target.src = "https://placehold.co/120x32/ea2e0e/ffffff?text=ONE+MAN"; }}
                />
                <span className="text-xl font-extrabold tracking-tight text-gray-800">ONE MAN</span>
            </div>
          ...
          <nav className="space-y-6">
            <Link to="#" onClick={toggleNavDrawer} className="block text-lg font-medium text-gray-700 hover:text-[#ea2e0e] uppercase">Men</Link>
            <Link to="#" onClick={toggleNavDrawer} className="block text-lg font-medium text-gray-700 hover:text-[#ea2e0e] uppercase">Women</Link>
            <Link to="#" onClick={toggleNavDrawer} className="block text-lg font-medium text-gray-700 hover:text-[#ea2e0e] uppercase">Top Wear</Link>
            <Link to="#" onClick={toggleNavDrawer} className="block text-lg font-medium text-gray-700 hover:text-[#ea2e0e] uppercase">Bottom Wear</Link>
            ...
            {isLoggedIn ? (
                <>
                    ...
                    >
                        <HiOutlineUser className="w-6 h-6 mr-2 text-[#ea2e0e]"/> My Profile
                    </Link>
                    <button
                        onClick={() => { toggleNavDrawer(); handleLogout(); }}
                        className="flex items-center text-lg font-medium text-gray-700 hover:text-[#ea2e0e] w-full text-left"
                    >
                         <HiOutlineArrowRightOnRectangle className="w-6 h-6 mr-2 text-red-500"/> Logout ({user?.name.split(' ')[0] || 'User'})
                    </button>
                </>
            ) : (
                <Link
                ...
                >
                    <span className="text-[#ea2e0e] font-bold">Sign In</span> / Sign Up
                </Link>
            )}
